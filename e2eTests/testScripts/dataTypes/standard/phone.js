	...
	describe('"phone"', function() {
		...
		it("Filter support", function(done){
			this.client
				...
				.execute(function(){
					...
					expect(ret.value).toEqual('x1234567890()- ');
					done();
				});
		});
		...
		it("Checking toField & fromField converters", function(done){
			this.client
				.execute(function(testChars){
					...
					return [
						{val: ifw.val(), equal: '234-567-8910x1234', because: "Should be set and converted"},
						{val: ifw.inputField('get'), equal: ifw.val(), because: "Should be retrieved without any conversion"}
					];
				}, '23456789101234', function(err,ret){
					app.batchExpect(ret.value);
					done();
				});
		});
		...
		it("Validation - initial flagging", function(done){
			this.client
				.moveToObject('input')
				.buttonDown(app.Buttons.LEFT).buttonUp(app.Buttons.LEFT) //click
				.keys('234567891') //not enough numbers, need 1 more
		...
		var correctInputs = ['2-()(345(67-8  910', '2345678910x1234', '12345678910x1234', '5678910x1234', '5678910'];
		var invalidInputs = ['2-()(x345(67-8  910', '234567891x01234', '2345678910x123x4'];
