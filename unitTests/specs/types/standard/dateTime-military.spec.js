 ...
 */
 ...
 describe('The dateTime-military data-type', function(){
	...
	it('can be created with a placeholder and a calendar addon', function(){
		var input = $('<input type="text" data-type="dateTime" data-hours= "true"/>').wrap('<div/>').inputField();	

		expect(input.parent().siblings('.addon').length).toBe(1);
		expect(input.parent().children('.placeholder').length).toBe(1);
		expect(input.parent().children('.placeholder').text()).toBe('MM/DD/YYYY HH:MM');
	...
	it('has a calender addon that can be manipulated', function(done){
		var input = $('<input type="text" data-type="dateTime" data-hours= "true"/>').appendTo(testContainer).inputField();	
		...
		.then(function(){
			...
			return pause(500); // Wait for fade out 
		})
		.then(function(){
