...
* **dateTime** - *For entering dates and times in the 'MM/DD/YYYY h:MM AM/PM* format. Works the same way as date, but also checks for a valid time. The calendar picker will set the time to noon.
    - The attribute 'data-hours' can be used to have date-time be set to military time. It just must be included and can be set to any arbitrary value, as long at the form field is built with that attribtue then the time will be in military format. The time will now be set to 24 hour time and will not include AM/PM signifiers at the end 
