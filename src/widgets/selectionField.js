...
$.widget('formBuilder.selectionField', {
	...
	_create: function() {
		 ...
		 */
		const parentContainer = field.closest('.selection-field-group, :formBuilder-formBuilder');
		...
		if(self.isRadio) {
			...
			const widgets = self.radioGroup.not(e).filter(':formBuilder-selectionField');
			if(!o.required) {
